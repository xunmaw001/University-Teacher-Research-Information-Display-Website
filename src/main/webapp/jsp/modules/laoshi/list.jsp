                        ...
                        <div class="table-responsive mb-3">
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th onclick="sort('name')">用户名称</th>
                                    <th onclick="sort('sexTypes')">性别</th>
                                    <th onclick="sort('phone')">联系方式</th>
                                    <th onclick="sort('xyTypes')">学院</th>
                                    <th onclick="sort('yxTypes')">院系</th>
                                    <th onclick="sort('area')">地区</th>
                                    <th onclick="sort('lwFile')">论文</th>
                                    <th>头像</th>
                                    <th>操作</th>
                                </tr>
                                </thead>
...
<script>
    ...
    function setDataRow(item, number) {
        ...
        var yxTypesCell = document.createElement('td');
        for (var i = 0; i < yxTypesOptions.length; i++) {
            if(yxTypesOptions[i].id == item.yxTypes){//下拉框value对比,如果一致就赋值汉字
                    yxTypesCell.innerHTML = yxTypesOptions[i].name;
            }
        }
        row.appendChild(yxTypesCell);

        var areaCell = document.createElement('td');
        areaCell.innerHTML = item.area;
        row.appendChild(areaCell);

        var lwFileCell  = document.createElement('td');

        if(item.lwFile != null){
            var lwFileFile = document.createElement('button');
            lwFileFile.innerHTML = '下载';
            varlwFileFileAttr = "download('" + item.lwFile + "')"
            lwFileFile.setAttribute('onclick',varlwFileFileAttr);
            lwFileCell.appendChild(lwFileFile);
        }else{
            var lwFileFile = document.createElement('span');
            lwFileFile.innerHTML = '暂时未上传文件';
            lwFileCell.appendChild(lwFileFile);
        }

		row.appendChild(lwFileCell);
    ...
    function remove(id) {
        ...
        if (mymessage == true) {
            ...
            httpJson("laoshi/delete", "POST", paramArray, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    //查询当前页面下所有列表
        ...
        function yxTypesSelect() {
            //填充下拉框选项
            http("yuanxi/page?page=1&limit=100&sort=&order=&dicCode=yx_types", "GET", {}, (res) => {
                if(res.code == 0){
    ...
    function crossTable(id, crossTableName) {
        ...
        window.sessionStorage.setItem('crossTableName', "laoshi");
