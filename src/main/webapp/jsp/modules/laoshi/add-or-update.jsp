                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                                   ...
                                   <div class="form-group col-md-6">
                                       <label>院系</label>
                                       <select id="xyTypesSelect" name="xyTypes" class="form-control">
                                       </select>
                                   </div>
                                   <div class="form-group col-md-6">
                                       <label>院系</label>
                                       <select id="yxTypesSelect" name="yxTypes" class="form-control">
                                       </select>
                                   </div>
                                    <div class="form-group col-md-6">
                                        <label>地区</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>论文</label>
                                        <input name="file" type="file" class="form-control-file" id="lwFileupload">
                                        <label id="lwFileName"></label>
                                        <input name="lwFile" id="lwFile-input" type="hidden">
                                    </div>
                                   <div class="form-group col-md-6">
                                       <label>头像</label>
...
<script>
    ...
    function upload() {
        $('#lwFileupload').fileupload({
            ...
            dataType: 'json',
                ...
                document.getElementById('lwFile-input').setAttribute('value', baseUrl + 'file/download?fileName=' + data.result.file);
                document.getElementById('lwFileName').innerHTML = "上传成功!";
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("laoshi/" + urlParam, "POST", data, (res) => {
                if(res.code == 0
        )
            {
    ...
    //查询当前页面下所有列表
        ...
        function xyTypesSelect() {
            //填充下拉框选项
            http("xueyuan/page?page=1&limit=100", "GET", {}, (res) => {
                if(res.code == 0){
                    xyTypesOptions = res.data.list;
            }
        });
        }
        function yxTypesSelect() {
            //填充下拉框选项
            http("yuanxi/page?page=1&limit=100", "GET", {}, (res) => {
                if(res.code == 0){
                    yxTypesOptions = res.data.list;
            }
        });
        }
    ...
    function initializationXytypesSelect(){
        var xyTypesSelect = document.getElementById('xyTypesSelect');
            for (var i = 0; i < xyTypesOptions.length; i++) {
                xyTypesSelect.add(new Option(xyTypesOptions[i].name,xyTypesOptions[i].id));
            }
    }
    function initializationYxtypesSelect(){
        var yxTypesSelect = document.getElementById('yxTypesSelect');
            for (var i = 0; i < yxTypesOptions.length; i++) {
                yxTypesSelect.add(new Option(yxTypesOptions[i].name,yxTypesOptions[i].id));
            }
    }
    // 下拉框选项回显
    function setSelectOption() {
        ...
        for (var i = 0; i < xyTypesOptions.length; i++) {
            if(xyTypesOptions[i].id == ruleForm.xyTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("xyTypesSelect").options[i].selected = true;
            }
        }
        for (var i = 0; i < yxTypesOptions.length; i++) {
            if(yxTypesOptions[i].id == ruleForm.yxTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("yxTypesSelect").options[i].selected = true;
            }
        }
    }
    ...
    function dataBind() {
        ...
        $("#area").val(ruleForm.area);
        $("#lwFile").val(ruleForm.lwFile);
    ...
    $(document).ready(function () {
        ...
        initializationXytypesSelect();
        initializationYxtypesSelect();
        getDetails();
        //初始化上传按钮
        upload();
        //单列求和
